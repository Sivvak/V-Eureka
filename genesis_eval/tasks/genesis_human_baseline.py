...
class Go2Env:
    def __init__(
        ...
        self.reward_functions, self.episode_sums, self.reward_dicts = (
            dict(),
            dict(),
            dict(),
        )
    ...
    def reset_idx(self, envs_idx):
        ...
        for rew_state in self.reward_dicts.keys():
            self.extras["episode"]["rew_" + rew_state] = torch.mean(
                self.reward_dicts.get(rew_state, 0.0)
            ).item()

        self.extras["episode"]["consecutive_successes"] = (
            self.consecutive_successes.mean()
        )
    ...
    def _reward_eureka(self) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
        ...
        reward_eureka = (
            1.0 * _reward_tracking_lin_vel(self)
            + 0.2 * _reward_tracking_ang_vel(self)
            + -1.0 * _reward_lin_vel_z(self)
            + -0.005 * _reward_action_rate(self)
            + -0.1 * _reward_similar_to_default(self)
            + -50.0 * _reward_base_height(self)
        )

        return reward_eureka, {"reward_eureka": reward_eureka}
